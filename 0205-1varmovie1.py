...
#CSVファイルを入力
with open("0205.1.csv") as csvfile:
...
with open("0205.1.with_var.csv", 'w') as f:
    writer = csv.writer(f)
    writer.writerows(l)
...
def animate(k):
    ...
    #ax.plot( x[0:k], y[0:k],linestyle='solid',color="red",marker="o")
    plt.title("var_fvangle(0205.1)"+timestamp[k])
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-11),interval=56.6398,repeat = True)
ani.save('var_fvangle(0205.1)interval=56.6398.mp4', writer='ffmpeg')
#plt.show
