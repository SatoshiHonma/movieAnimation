...
"""
with open(r'/Users/honmasatoshi/OneDrive - 東京理科大学/研究データ/本間実験依頼/頷きアンケート整理/CIMG0837anke.csv') as f:
    #print(f.read())
    reader = csv.reader(f)
...
def animate(k): #1
    ...
    for p in range(1,k):
        goukei += int(ya[p])
    plt.title("anke(CIMG0837)honma"+timestamp[k]+"total"+str(goukei))
...
ani.save('anke(CIMG0837)honma.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #2
    ...
    for p in range(1,k):
        goukei += int(yb[p])
    plt.title("anke(CIMG0837)iida"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0837)iida.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #3
    ...
    for p in range(1,k):
        goukei += int(yc[p])
    plt.title("anke(CIMG0837)sekiguchi"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0837)sekiguchi.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #4
    ...
    # 描画時点からの時刻基準で未来方向の点群を描画してしまうけど、まあいいか…
    if k<300:
        ax.plot( x[0:k], yd[0:k],linestyle='solid',color="red",marker="o")
    if k>=300:
        ax.plot( x[k-300:k], yd[k-300:k],linestyle='solid',color="red",marker="o")
    #ax.plot( x[0:k], y[0:k],linestyle='solid',color="red",marker="o")
    goukei = 0
    for p in range(1,k):
        goukei += int(yd[p])
    plt.title("anke(CIMG0837)kamiyama"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0837)kamiyama.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #5
    ...
    # 描画時点からの時刻基準で未来方向の点群を描画してしまうけど、まあいいか…
    if k<300:
        ax.plot( x[0:k], ye[0:k],linestyle='solid',color="red",marker="o")
    if k>=300:
        ax.plot( x[k-300:k], ye[k-300:k],linestyle='solid',color="red",marker="o")
    #ax.plot( x[0:k], y[0:k],linestyle='solid',color="red",marker="o")
    goukei = 0
    for p in range(1,k):
        goukei += int(ye[p])
    plt.title("anke(CIMG0837)kudou"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0837)kudou.mp4', writer='ffmpeg')
#plt.show
