...
"""
with open(r'/Users/honmasatoshi/OneDrive - 東京理科大学/研究データ/本間実験依頼/頷きアンケート整理/CIMG0839anke.csv') as f:
    #print(f.read())
    reader = csv.reader(f)
...
def animate(k): #1
    ...
    for p in range(1,k):
        goukei += int(ya[p])
    plt.title("anke(CIMG0839)honma"+timestamp[k]+"total"+str(goukei))
...
ani.save('anke(CIMG0839)honma.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #2
    ...
    for p in range(1,k):
        goukei += int(yb[p])
    plt.title("anke(CIMG0839)esaka"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0839)esaka.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #3
    ...
    for p in range(1,k):
        goukei += int(yc[p])
    plt.title("anke(CIMG0839)huruhata"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0839)huruhata.mp4', writer='ffmpeg')
#plt.show
...
def animate(k): #4
    ...
    for p in range(1,k):
        goukei += int(yd[p])
    plt.title("anke(CIMG0839)tanizaki"+timestamp[k]+"total"+str(goukei))
#ani =animation.FuncAnimation(fig, animate, frames=range(0,8999),interval=33.3,repeat = True,blit=True)
ani =animation.FuncAnimation(fig, animate, frames=range(0,len(l)-2),interval=1000,repeat = True)
ani.save('anke(CIMG0839)tanizaki.mp4', writer='ffmpeg')
#plt.show
